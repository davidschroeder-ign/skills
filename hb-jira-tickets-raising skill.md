...
1. Estimate using the t-shirt scale: `0.25`, `0.5`, `1`, `2`, `3` (rarely higher — if it feels bigger than a 3, that's a signal the ticket should probably be split rather than estimated as one large item).
2. Base the estimate on what's evident from the ticket itself: number of flows/pages touched, whether it's isolated (single component, clear repro) vs. cross-cutting (multiple flows, unclear root cause, needs investigation), whether it's a straightforward fix pattern vs. something requiring design/backend coordination.
...
| **Story Points** | carried over from Step 1b's estimate | `customfield_14097` | plain number, e.g. `0.5`, `1`, `2`, `3` — reuse the estimate already stated in the Description per Step 1b, don't re-derive it here |
