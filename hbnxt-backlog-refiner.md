...
a conversation, this skill sweeps the HBNXT backlog, reformats every unrefined ticket
in place, and reports out — designed to be triggered by a scheduled routine with no
human in the loop per run.
...
Send a DM via the Chat MCP (`chat_send_message_as_agent`, falling back to
`chat_send_message_as_user` if the agent send is unavailable) once the whole sweep is
done — one message per run, not one per ticket.
...
No unrefined backlog tickets found this run.
```
