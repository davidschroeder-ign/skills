...
Depends on **hb-jira-tickets-raising-skill** for the actual ticket format, story-point
estimation (Step 1b), priority/team assignment (Step 6), and consolidation logic — this
skill does not redefine those; it calls them.
...
| Backlog JQL | `project = HBNXT AND status = "Screening" AND issuetype in (Bug, Task) AND (labels is EMPTY OR labels != "dug-ticket-refined") AND assignee = "david_schroeder@ign.com" ORDER BY created ASC` |
...
- The `assignee` clause scopes this run to David's tickets only — this skill does not
  groom the full team backlog. If it needs to run for a different owner, update this
  constant's assignee value rather than removing the clause.
...
Apply `hb-jira-tickets-raising-skill` (Step 1's intake checklist through Step 2's ticket
format, Step 1b's story-point estimate, and Step 6's priority/team field table) using the
original body as the sole input — there is no reporter to ask follow-up questions of, so:
...
- Still estimate story points per hb-jira-tickets-raising-skill Step 1b (Ideal Days, 1 point
  = 1 ideal day, 3-day hard cap; reason stated as the first line of the Description) — this applies to every ticket, not
  just direct-filing. Still assign priority and team per Step 6's "Standing fields for
  direct filing" table (`customfield_12300` for team, `priority` per its
  default-to-Emergency rule). If genuinely unscoreable from the existing content (e.g.
  can't tell CE from P&B), use `Unscreened` priority or note the team ambiguity as a gap,
  and use Step 1b's "Unscored" fallback for story points rather than guessing either one.
- Still apply Critical Rule 8/10 (consolidation against the "known tickets already on
  file" list and the Step 1a UAT duplicate check) — if a ticket looks like a near-duplicate,
  note that as an action item rather than merging it unilaterally; this skill does not
  delete or merge tickets on its own.
...
1. `Atlassian:editJiraIssue` — set `description` to the Step 4 body (which already carries
   the story-point rationale as the first line of its Description per Step 1b), and set
   priority/team (`customfield_12300`) per hb-jira-tickets-raising-skill Step 6's field
   table plus story points (`customfield_14097`) from the Step 1b estimate.
