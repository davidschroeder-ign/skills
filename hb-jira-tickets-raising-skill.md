...
## Step 1a — UAT-period duplicate check (mandatory, every ticket)

The UAT period began **2026-08-18**. Before drafting any ticket, search Jira for tickets resolved since UAT started that might describe the same underlying issue — the "Known tickets already on file" list in Project context is a manually maintained snapshot and goes stale fast; this live check catches what it misses.

1. Run a Jira search scoped to `project = HBNXT AND status = Done AND resolutiondate >= "2026-08-18"` (adjust the status clause to match whatever the board calls its closed/done state if it differs). Do this via the Jira search tool — don't rely on memory or the static "Known tickets" list alone.
2. Compare the new ticket's core symptom (page/flow, error text, component) against the summaries/descriptions of what comes back — not just title keyword overlap. A ticket titled differently can still be the same root issue (e.g. "Change Plan fails" vs. "RangeError on subscription update").
3. **If a plausible match is found:** don't silently fold it in or silently proceed as if it's new. Add the duplicate banner (see Step 2 format) as the very first line of the output, above the title, naming the matching ticket key, its summary, and its resolution date. Still draft the full ticket underneath — the banner flags it for human judgment, it doesn't block drafting.
4. **If no match is found:** proceed to draft normally, no banner line. Don't fabricate a "checked, none found" note — the absence of the banner is the signal.
5. If the Jira search tool/connection is unavailable, say so explicitly and fall back to checking against the static "Known tickets already on file" list — don't silently skip the duplicate check without telling the reporter you couldn't run it live.

This check is separate from and in addition to Critical Rule 8 (consolidate over-fragmenting) — that rule governs merging overlapping *new* requests together; this step governs cross-checking against tickets already closed out during UAT.

## Step 1b — Story point estimate (mandatory, every ticket)

Every ticket gets a story-point estimate — this is no longer limited to the Step 6 direct-filing workflow.

1. Estimate in **Ideal Days** — 1 story point = 1 ideal day of focused, uninterrupted effort to complete the work, not elapsed calendar time (a 2-point ticket might take 4 real days once meetings/reviews are factored in). Use: `0.25`, `0.5`, `1`, `2`, `3`. **3 is a hard cap, not a soft one** — no ticket should be estimated above 3 ideal days; if it feels bigger than that, split it into smaller tickets instead of estimating it as one large item. See [Story Point Estimation Using Ideal Days](https://humblebundle.atlassian.net/wiki/spaces/HND/pages/2216820754/Story+Point+Estimation+Using+Ideal+Days).
2. Base the estimate on the focused effort the fix itself requires — number of flows/pages touched, whether it's isolated (single component, clear repro) vs. cross-cutting (multiple flows, unclear root cause, needs investigation), whether it's a straightforward fix pattern vs. something requiring design/backend coordination. Don't inflate for surrounding noise (meetings, review turnaround, QA wait time) — ideal days measure the work itself, not the calendar it happens on.
3. State the estimate and a one-line reason as the **first line of the Description section** (see Step 2 format) — never set it silently with no explanation. This is a draft-time estimate for the assignee to adjust, not a final commitment.
4. If the ticket genuinely can't be sized from available information (e.g. root cause totally unknown, scope depends on an open question), say so explicitly instead of guessing a number — e.g. "**Story Points: Unscored** — scope depends on whether this is isolated to Change Plan or affects all subscription-update flows; confirm before sizing."
5. When this skill is used for direct filing (Step 6), the same estimate carries over into `customfield_14097` — Step 6 does not re-derive it separately. **When invoked directly in conversation (not by `hbnxt-backlog-refiner`), showing the estimate in the draft is not the same as writing it to Jira** — see the confirmation gate in Step 6 before `customfield_14097` is ever actually set on a real issue. `hbnxt-backlog-refiner` is exempt from this gate since it runs unattended with no reporter to confirm with.

## Step 2 — Ticket format

```
[If a UAT duplicate was found in Step 1a, this banner line goes here, above the title:]
⚠️ **Possible duplicate of HBNXT-XXXX** — "[matching ticket summary]" (resolved [date], during UAT) — confirm before filing as new.
...
**Story Points: [estimate]** — [one-line reason per Step 1b, e.g. "isolated single-flow fix,
no new UI" or "cross-cutting across guest and auth checkout, needs backend coordination"].
...
Reference note only (e.g. "Reference screenshot / recording attached", or "Not provided" if none).

---

# Test Data

- Env: defaults to `UAT` unless the reporter states or implies otherwise (see Environments note below); mark any genuinely conflicting/unclear case "Not verified"
...
- **Types:** `BUG`, `TASK`
- **Environments:** `DEV`, `STG`, `DEV+STG`, `UAT`
- **Default environment: `UAT`.** The team is in the UAT period (started 2026-08-18) — assume `UAT` for the title tag and Test Data `Env` field unless the reporter's message states or clearly implies a different environment (e.g. explicitly says "on dev" or "on staging"). Don't stop to ask which environment a ticket is for; only note it as unconfirmed if the reporter gives conflicting environment signals.
...
**When filing directly to Jira (Step 6), the `#` section headers and `---` dividers above must land as real Jira formatting, not literal text.** Build the description as an ADF document (`contentFormat: adf`) with an actual `heading` node (`"level": 1`) for every section title (Description, Steps to Reproduce, Expected Result, Actual Result, Evidence, Test Data, Acceptance Criteria) and an actual `rule` node between sections in place of each `---`. Don't rely on a tool's plain-text/markdown auto-conversion for this — it has rendered `---` as the literal characters "—" in a paragraph rather than a horizontal rule, and headers can silently fall back to plain bold text instead of a true heading depending on the tool. If a plain-text/markdown path is the only option available, verify the rendered result (e.g. re-fetch the issue) before treating the ticket as done, and switch to building ADF directly if headings or dividers didn't come through as real formatting.
...
2. **Steps to Reproduce, Expected Result, Actual Result, and Acceptance Criteria are each their own separate section, each with its own `#` heading and a `---` divider before it.** Don't merge Steps to Reproduce into the AC, and don't fold Expected/Actual into the Description — each carries distinct information and should stay separately headed. When filing directly to Jira, these must be real Heading 1 nodes and real divider (`rule`) nodes, not plain text that merely looks like a heading or a dashed line — see the note at the end of Step 2.
...
10. **UAT duplicate check runs every time (Step 1a).** Every ticket gets checked against tickets resolved since UAT start (2026-08-18) before drafting. If a plausible duplicate turns up, the ⚠️ banner goes at the very top of the output, above the title — never buried mid-ticket, never dropped silently.
11. **Story points are estimated on every ticket (Step 1b).** The estimate and its one-line reason are always the first line of the Description — never omitted, never set without stating why. **In direct conversation use, the estimate showing up in the draft is not the same as writing it to Jira** — when the skill goes on to file via Step 6, `customfield_14097` isn't set until the reporter has confirmed the full ticket, same gate as every other field. (This gate doesn't apply to `hbnxt-backlog-refiner`, which runs unattended and writes directly — see that skill's own Step 5.)
...
| **Story Points** | carried over from Step 1b's estimate (Ideal Days: 1 point = 1 ideal day) | `customfield_14097` | plain number, e.g. `0.25`, `0.5`, `1`, `2`, `3` (3 is the hard cap) — reuse the estimate already stated in the Description per Step 1b, don't re-derive it here |
...
2. Decide the remaining optional fields (Epic link only if obvious, Priority per the rule above, Assignee confirmed if unspecified) — Story Points is already set from Step 1b, don't re-estimate it.
3. **Show the full draft — description (with the Step 1b story-point line already in it), steps to reproduce, every field you're defaulting — and get explicit confirmation on the whole ticket before filing.** The story-point estimate appearing in the draft text is not a write; nothing is written to Jira, including `customfield_14097`, until the reporter has confirmed the ticket as a whole. Don't file on the first pass, and don't set the story-points field early or separately from the rest of the fields.
4. Once confirmed, call the Jira ticket-creation tool with the base params plus an `additional_fields` object carrying the custom fields above (team, story points, sprint) and a top-level `parent` key only when an Epic link applies — this is the first and only point at which `customfield_14097` is actually set.
...
- Run the Step 1a UAT duplicate check (tickets resolved since 2026-08-18) before every draft, no exceptions — this is separate from and in addition to the static "Known tickets already on file" list below.
- Run the Step 1b story-point estimate before every draft, no exceptions — the estimate and its reason are always the first line of the Description.
